(function(){
	...
	var Debugger = NEScript.Debugger = function(emulatorRef){
		...
		this.refMainMemory = this.EMULATOR.CPU._mainMemory._memory;
	...
	Debugger.prototype.step = function(){
		this.EMULATOR.step();
		this.updateInfo();
		this.disassembleChunk(this.EMULATOR.CPU._regPC[0]);
	}

	Debugger.prototype.run = function(){
		while(NEScript.IS_RUNNING){
			if(EMULATOR.CPU._regPC[0] === this.breakpointVal){
				break;
			}
			this.EMULATOR.step();
		}
	...
	Debugger.prototype.updateInfo = function(){
		this.regA.value = this.EMULATOR.CPU._regs[0].toString(16);
		this.regX.value = this.EMULATOR.CPU._regs[1].toString(16);
		this.regY.value = this.EMULATOR.CPU._regs[2].toString(16);
		this.regP.value = this.EMULATOR.CPU.flagsToP().toString(2);
		this.regSP.value = this.EMULATOR.CPU._regs[4].toString(16);
		this.regPC.value = this.EMULATOR.CPU._regPC[0].toString(16);
	...
	Debugger.prototype.updateDisassembly = function(){
		...
		for(i = 0x8000; i < 0xFFFA; i++){
			...
			opcode = this.EMULATOR.CPU._mainMemory._memory[i];
			...
			if(numOperands === 2){
				lobyte = this.EMULATOR.CPU._mainMemory._memory[i+1];
				if(lobyte < 0x10){
				...
				hibyte = this.EMULATOR.CPU._mainMemory._memory[i+2];
				if(hibyte < 0x10){
	...
	Debugger.prototype.readCPUWord = function(addr){
		var lobyte = refMainMemory[addr];
		var hibyte = regMainMemory[addr+1];
	...
	Debugger.prototype.updateMemory = function(start, limit){
		...
		for(i = start; i < limit; i += 8){
			...
			for(j = 0; j < 8; j++){
				tmpVal = this.EMULATOR.CPU._mainMemory._memory[i + j];
